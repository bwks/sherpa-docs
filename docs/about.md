...
and manage the lifecycle with a few commands. Vagrant also made sharing
labs with peers much easier.

Docker came along and it's great when you can get by with only using
Containers. I came from a networking background and in that space, VM's
are mostly still king.
